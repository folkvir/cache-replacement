...
describe('Testing the MRU policy', function() {
  it('should return undefined when no element in the cache', async function() {
    ...
    assert.deepEqual(await cache.get('titi'), undefined);
  });
  ...
  it('should correctly delete the most recently used element [method get] (cache size =2)', async function() {
    let cr = new CacheReplacementPolicy();
    ...
    const r4 = await cache.get('iti');
    assert.deepEqual(r4, undefined);
    assert.deepEqual(await cache.size(), 2);
  });
  it('should correctly delete the most recently used element [method has] (cache size =2)', async function() {
    let cr = new CacheReplacementPolicy();
    let cache = cr.createCache(Cache, {max: 2});
    cr.setPolicy('mru', cache)
    const r = await cache.set('titi', 42);
    ...
    const r3 = await cache.set('tata', 44);
    const r4 = await cache.get('titi');
    assert.deepEqual(r2, true);
    assert.deepEqual(r4, undefined);
    assert.deepEqual(await cache.size(), 2);
  });
  it('should correctly delete the most recently used element [method set] (cache size =3)', async function() {
    let cr = new CacheReplacementPolicy();
