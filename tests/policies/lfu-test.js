...
describe('Testing the LFU policy', function () {
  ...
  it('should correctly delete the least frequently used element [method set] (cache size =3)', function () {
    ...
    cache.set('titi', 42)
    print(cache)
    assert.deepEqual(cache.leastFrequent, 'titi')
    ...
    cache.set('titi', 56)
    print(cache)
    assert.deepEqual(cache.leastFrequent, 'toto')
    const r3 = cache.set('tata', 44)
    assert.equal(r3, true)
    print(cache)
    assert.deepEqual(cache.leastFrequent, 'toto')
    assert.deepEqual(cache.mostFrequent, 'titi')
    cache.set('tutu', "Qu'est-ce qu'un élémentaire de fromage ? un emmental !")

    assert.deepEqual(cache.leastFrequent, 'tata')
    assert.deepEqual(cache.mostFrequent, 'titi')
    ...
    assert.deepEqual(cache.leastFrequent, 'tata')
    assert.deepEqual(cache.mostFrequent, 'tutu')
